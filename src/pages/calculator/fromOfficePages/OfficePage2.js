  ...
  return (
    <>
      <MultiModeModal
        ...
        setVehicleNames={props.setVehicles}
      ...
      >
        ...
        <Typography variant="h6" sx={{ ml: "10px" }}>
          Select travel mode/s used throughout the week
        </Typography>
      </Box>
      <Box
        sx={{
      ...
      <Box sx={{ mt: "30px" }}>
        <Tooltip title="Select if you use more than one travel mode in a single trip to work. e.g. Drive to the bus stop, then take the bus">
          <Link href="#" onClick={openModal}>
            {Object.keys(props.multiVehicle).length === 0
              ? "I use two modes in one trip"
              : "Update multi mode trip"}
          </Link>
        </Tooltip>
      </Box>
    </>
